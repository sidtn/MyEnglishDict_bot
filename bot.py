...
async def process_start_command(msg: types.Message):
    await msg.reply(
        ...
        "/test - training the words from your dictionary;\n"
...
async def user_request(call: types.CallbackQuery):
    ...
    button = types.InlineKeyboardButton(
        "Add this user", callback_data=f"adduser_{newuser[0]}"
    )
    keyboard.add(button)
    await bot.send_message(
        ADMIN_ID, f"User {newuser[1]} want to join.", reply_markup=keyboard
    )
...
async def add_user(call: types.CallbackQuery):
    ...
    await bot.delete_message(call.from_user.id, message_id=call.message.message_id)
    await bot.send_message(
        call.from_user.id, f"The user with id {user_id} has been added."
    )
...
async def delete_words(msg: types.Message):
    ...
    if words:
        for word in reversed(words):
            keyboard = types.InlineKeyboardMarkup()
            button = types.InlineKeyboardButton(
                "Delete", callback_data=f"del_{word[0]}"
            )
            keyboard.add(button)
...
async def words_trainer(msg: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data["name"] = msg
    ...
    if testdata:
        ...
        keyboard.add(buttons_list[0], buttons_list[1])
        keyboard.add(buttons_list[2], buttons_list[3])
        await bot.send_message(
            msg.from_user.id,
            f"<b>{testdata[0][1]}</b>",
...
async def show_gerund_or_inf(msg: types.Message):
    ...
    if result:
        await msg.answer(
            f'<b>{result[1]}</b> is used after the word <b>"{result[0]}"</b>',
            parse_mode=types.ParseMode.HTML,
        )
    else:
        await msg.answer(f"Word [{msg_text}] not found.")
...
@dp.message_handler(
    lambda msg: msg.text.startswith("/count") and msg.from_user.id in ALLOWED_USERS
)
async def show_count(msg: types.Message):
    count = db.get_count_words(msg.from_user.id)
    await msg.answer(
        f"There are <b>{str(count)}</b> words in your dictionary.",
        parse_mode=types.ParseMode.HTML,
    )
...
async def answer_translator(msg: types.Message):
    ...
    if word_in_db_user:
        ...
        await bot.send_voice(
            msg.from_user.id,
            voice=audio,
            caption=f"{word_in_db_user[0]} - {word_in_db_user[1]}",
        )
    elif word_in_db_all:
        ...
        await bot.send_voice(
            msg.from_user.id,
            voice=audio,
            caption=f"{word_in_db_all[0]} - {word_in_db_all[1]}",
        )
        ...
        if isinstance(result, str):
            ...
            if len(result[1].split(" ")) <= 3:
                ...
                await bot.send_voice(
                    msg.from_user.id, voice=audio, caption=f"{result[1]} - {result[0]}"
                )
            else:
...
async def input_from_unregistered_user(msg: types.Message):
    if msg.from_user.id not in ALLOWED_USERS:
        ...
        button = types.InlineKeyboardButton(
            "Click",
            callback_data=f"request_{str(msg.from_user.id)}_{msg.from_user.username}",
        )
        keyboard.add(button)
        await msg.answer(
            "If you want to translate and add words to the you dictionary, click on the button,"
            "and admin will receive your request.",
            reply_markup=keyboard,
        )
