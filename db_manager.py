...
class DbManage:
    ...
    def create_tables(self):
        ...
        self.__cursor.execute(
            "CREATE TABLE IF NOT EXISTS irregularverb "
    ...
    def get_users(self):
        userslist = [
            user[0]
            for user in self.__cursor.execute("SELECT user_id FROM users").fetchall()
        ]
        return userslist
    ...
    def add_word(self, user_id, word, translate):
        self.__cursor.execute(
            "INSERT INTO words (user_id, word, translate)" " VALUES(?, ?, ?);",
            (user_id, word, translate),
        )
        self.__conn.commit()
    ...
    def find_word(self, text, user_id=None):
        if user_id:
            ...
            result = self.__cursor.execute(
                "SELECT word, translate FROM words WHERE word LIKE (?)"
                "or translate LIKE (?);",
                (
                    text,
                    text,
    ...
    def get_word_for_test(self, for_user=None):
        all_records = self.__cursor.execute(
            "SELECT word, translate FROM words WHERE user_id=(?);", (for_user,)
        ).fetchall()
    ...
    def get_count_words(self, user_id):
        count = self.__cursor.execute(
            "SELECT count(word) FROM words WHERE user_id = (?)", (user_id,)
        ).fetchone()
        return count[0]

    def get_irregular_vebs(self, word):
        result = self.__cursor.execute(
            "SELECT * FROM irregularverb WHERE form1 LIKE (?)"
    ...
    def get_data_from_verbs(self, word):
        result = self.__cursor.execute(
            "SELECT word, verbform.form FROM verbs JOIN verbform ON"
            " verbs.form = verbform.id WHERE word LIKE (?);",
            (word,),
        )
        return result.fetchone()

    def get_words_for_delete(self, user_id):
        words = self.__cursor.execute(
            "SELECT word, translate FROM words WHERE user_id LIKE (?)"
    ...
    def del_record(self, word, user_id):
        record = self.__cursor.execute(
            "DELETE FROM words WHERE word LIKE (?) AND user_id = (?);",
            (
                word,
                user_id,
            ),
    ...
    def insert_data_irregular(self, data):
        for row in data:
            self.__cursor.execute(
                "INSERT OR IGNORE INTO irregularverb"
                "(form1, form2, form3, translate) VALUES(?, ?, ?, ?)",
                row,
            )
            self.__conn.commit()
    ...
    def insert_data_gerund(self, data):
        for num, list_words in enumerate(data, 1):
            for word in list_words:
                self.__cursor.execute(
                    "INSERT OR IGNORE INTO verbs(word, form)" " VALUES(?, ?)",
                    (word, num),
                )
                self.__conn.commit()
